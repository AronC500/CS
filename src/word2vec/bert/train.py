...
class BaseTrainer(object):
    ...
    def train(self, num_epochs, savemodel_path=r'record', recorder_path=r'record/recorder.json'):
...
class PREBERTTrainer(BaseTrainer):
    ...
    def print_record(self, train_loss, train_acc, val_loss, val_acc):
        ...
        f = open(r"record/prebert.log", 'a+')
        f.write('Epoch: {}/{}\n'.format(self.epoch, self.num_epochs - 1))
        f.write('Train: loss {:.4f}, acc_nsp {:.4f}, acc_mlm {:.4f}\n'.format(
            train_loss, train_acc[0], train_acc[1]))
        f.write('Val: loss {:.4f}, acc_nsp {:.4f}, acc_mlm {:.4f}\n'.format(
            val_loss, val_acc[0], val_acc[1]))
        f.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-hidden', '--hidden_size', type=int, default=200)
    ...
    # model
    config = BertConfig(hidden_size=args.hidden_size, vocab_size=len(vocab), num_attention_heads=8, num_hidden_layers=8,
                        return_dict=True)
    bert = PREBERT(config)
    if multi_gpu:
        bert = nn.DataParallel(bert, device_ids=[0, 1])
    # bert.load_state_dict(torch.load('/record/prebert_18.pth'))
    ...
    print('Device loaded: {}.'.format(trainer.device))
